...
# Outputs
scores, visfeat = detmodel.text_objdet(text_seq_batch, im_batch, bbox_batch,
    spatial_batch, num_vocab, embed_dim, lstm_dim, mlp_hidden_dims,
    vgg_dropout=False, mlp_dropout=False, grid_score=True, return_visfeat=True)
...
def run_on_image(im_path, query_list):
    im = skimage.io.imread(im_path)
    if im.ndim == 2:
        im = np.tile(im[..., np.newaxis], (1, 1, 3))
    proposal_bboxes = rpn_boxes.extract_proposal(sess, im_path)
    print('got %d RPN proposals' % len(proposal_bboxes))
    # add a "whole image" bounding box at the beginning
    im_h, im_w = im.shape[:2]
    proposal_bboxes = np.concatenate(([[0, 0, im_w, im_h]], proposal_bboxes))

    im_val, bbox_val, spatial_val = prepare_one_image_roi(im, im_mean, proposal_bboxes, min_size=600, max_size=1000)
    text_seq_val = np.zeros((T, len(query_list)), np.int32)
    for n_q, q in enumerate(query_list):
        text_seq_val[:, n_q] = text_processing.preprocess_sentence(q, vocab_dict, T)
    scores_val, visfeat_val = sess.run((scores, visfeat), {text_seq_batch: text_seq_val,
                                   im_batch: im_val,
                                   bbox_batch: bbox_val,
                                   spatial_batch: spatial_val})
    im_visfeat = visfeat_val[0]
    bbox_visfeat = visfeat_val
    return proposal_bboxes, scores_val, im_visfeat, bbox_visfeat

'''
def run_prediction(im_path, query_list):
    im = skimage.io.imread(im_path)
    if im.ndim == 2:
        im = np.tile(im[..., np.newaxis], (1, 1, 3))
