...
inline int get_block_num(int output_num, int kThreadsPerBlock) {
  const int maxBlocks = 65535;
  return min((output_num + kThreadsPerBlock - 1) / kThreadsPerBlock, maxBlocks);
}
    ...
    float* top_data, int* argmax_data, const Eigen::GpuDevice& d)
{
  ...
  ROIPoolForward<<<get_block_num(output_size, kThreadsPerBlock),
    ...
    float* bottom_diff, const int* argmax_data, const Eigen::GpuDevice& d)
{
  ...
  ROIPoolBackward<<<get_block_num(output_size, kThreadsPerBlock),
                       kThreadsPerBlock, 0, d.stream()>>>(
