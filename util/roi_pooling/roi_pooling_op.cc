...
REGISTER_OP("RoiPool")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      shape_inference::DimensionHandle num_channels = c->Dim(c->input(0), 3);
      shape_inference::DimensionHandle num_rois = c->Dim(c->input(1), 0);
      int pooled_height;
      int pooled_width;
      c->GetAttr("pooled_height", &pooled_height);
      c->GetAttr("pooled_width", &pooled_width);
      c->set_output(0, c->MakeShape({num_rois, pooled_height, pooled_width,
                                     num_channels}));
      return Status::OK();
    });
